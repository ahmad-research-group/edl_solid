...
ChemPotEqualHC::validParams()
{
    ...
    params.addRequiredParam<MaterialPropertyName>("mu0b", "The name of the second chemical potential");
...
//    params.addRequiredCoupledVar("conc", "concentration");
    ...
    params.addRequiredParam<MaterialPropertyName>("kT", "The name of the thermal energy");
    return params;
}
...
//        _eta_name(_var.name()),
        ...
        _kT(getADMaterialProperty<Real>("kT"))
...
{
    ADReal _mua = _mu0a[_qp] + _kT[_qp] * std::log(_u[_qp]/(1.0-_u[_qp])) + _za[_qp] * 1.602e-19 * _phi[_qp];
    ADReal _mub = _mu0b[_qp] + _kT[_qp] * std::log(_cb[_qp]/(1.0-_cb[_qp])) + _zb[_qp] * 1.602e-19 * _phi[_qp]; 
